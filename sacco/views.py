...
def superlist(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            '''
            above line of code displays a supervisor/user registered in a specific sacco or orgamisation by counter1
            '''
            user.save()
...
def delete_supervisor(request, supervisorID):
    '''
    View function that enables one delete a given supervisor/user in a sacco/organisation
    '''
...
def delete_sacco(request, saccoID):
    '''
    View function that enables deletion of a sacco/org instance
    '''
...
def saccoSupervisors(request):
    '''
    View function to retrieve all sacco/org supervisor/users instances
    '''
